...
/**
 * The Resource class represents the record of: 1. a resource stored by the
 * Server; 2. a resource sent to a server as part of a PUBLISH/REMOVE/SHARE
 * command; 3. a resource template sent to a server as part of a QUERY command;
 ...
 * and compared against other Resource objects - the tuple (channel, uri) is
 * used as the PrimaryKey to identify a Resource object.
 * 
 * Note: resourceSize is only set for a resource response. Where resourceSize is
 * null, it will be disregarded when converting to JSON for communication.
 */

// TODO AZ: the class seems to be doing too much, maybe resource response and
// template
// be split off into their own subclass with methods that handles look-up and
// copying
...
public class Resource extends JsonModel {
	...
	public static final String HIDDEN_OWNER = "*";
	...
	/**
	 * Initialize a Resource object based on the client arguments - if an option
	 * is optional and not set in the client argument, the default value for
	 * that option is used in the construction.
	 * 
	 * @param clientArgs
	 *            a list of client arguments parsed against EZShare server
	 *            commands/ options
	 */
	public Resource(ClientArgs clientArgs) {
		...
		// need to enable parsing null fields when converting resource object to
		// JSON!
		// without breaking resourceSize at the same time (this should be
		// ignored)
		ezserver = clientArgs.getOptionValue(ClientArgs.EZSERVER_OPTION, DEFAULT_EZSERVER);
	}

	/**
	 * The addTags method adds all comma-delimited tags in the input string to
	 * the calling Resource object's tags list.
	 * 
	 * @param tags_string
	 ...
	 * Resource objects (i.e. converted by fromJson method, where owner may be
	 * null).
	 * 
