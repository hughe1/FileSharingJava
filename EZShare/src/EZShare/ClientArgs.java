...
/**
 * The ClientArgs class represents a list of command-line arguments parsed
...
public class ClientArgs extends ArgsManager {
	 ...
	 * commands descriptor to initialize the object's CommandLine variable
	 * (cmd).
	 * 
	 * @param args
	 */
	...
	/**
	 * The getCommandOption method returns the server command (i.e. publish,
	 * share, query, fetch, exchange, remove) that is set by the client
	 * arguments. If more than one command option is set, "INVALID" will be
	 * returned.
	 * 
	 * @return the server command option set in the calling CliengArgs object.
	 *         If more than one command option is used, "INVALID" will be
	 *         returned.
	 */
	 ...
	 * 
	 * @return the server port specified in the client arguments, if a port was
	...
	public String getSafeHost() {
		if (!this.hasOption(HOST_OPTION)) {
			...
			try {
				...
				// default if failed to retrieve OS host name
				return "localhost";
			}
		}
	...
	/**
	 * The getSafeRelay method returns the relay value specified in the
	 * command-line arguments
	 * 
